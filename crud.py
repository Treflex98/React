...
def update_movie(db: Session, movie: schemas.Movie):
    db_movie = _get_movies_by_predicate(models.Movie.id == movie.id, db=db).first()
...
def delete_movie(db: Session, movie_id: int):
     db_movie = _get_movies_by_predicate(models.Movie.id == movie_id, db=db).first()
     if db_movie is not None:
...
def update_star(db: Session, star: schemas.Star):
    db_star = _get_stars_by_predicate(models.Star.id == star.id, db=db).first()
...
def delete_star(db: Session, star_id: int):
     db_star = _get_stars_by_predicate(models.Star.id == star_id, db=db).first()
     if db_star is not None:
         # delete object from ORM
         db.delete(db_star)
         # validate delete in db
...
def get_movies_stats_by_year(db: Session):
    ...
    func.avg(models.Movie.duration)).group_by(models.Movie.year).order_by(models.Movie.year).all()
